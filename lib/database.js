...
async function save_tx_array(arr_tx, blockheight, blockhash, cb)
{
  lib.get_rawtransactionfast(arr_tx, blockhash, async function(tx){
    if (tx != 'There was an error. Check your console.') {
      ...
      for(txit = 0; txit < tx.length; txit++){
        console.log("block %s, txid: %s", blockheight, tx[txit].txid)
        vin = await lib.prepare_vin_async(tx[txit]);
        console.log(vin);
        voutarr = await lib.prepare_vout_async(tx[txit].vout, tx[txit].txid, vin);
        //console.log(voutarr)
        vout = voutarr[0];
        nvin = voutarr[1];
        console.log("vinLength: %s, txid: %s", vin.length, tx[txit].txid);
        for(i=0;i<vin.length; i++){
          await update_address_async(nvin[i].addresses, blockheight, tx[txit].txid, nvin[i].amount, 'vin');
        }
        
        console.log("voutLength: %s, txid: %s", vin.length, tx[txit].txid);
        for(t=0;t<vout.length;t++){
          if (vout[t].addresses) {
            await update_address_async(vout[t].addresses, blockheight, tx[txit].txid, vout[t].amount, 'vout');
          }
        }
        total = await lib.calculate_total_async(vout);
        var newTx = new Tx({
          txid: tx[txit].txid,
          vin: nvin,
          vout: vout,
        ...
        saved = await newTx.save(function(err) {
          if (err) {
            return err
          } else {
            //console.log('txid: ');
