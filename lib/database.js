...
async function save_tx_array(arr_tx, blockheight, blockhash, cb)
{
  lib.get_rawtransactionfast(arr_tx, blockhash, function(tx){
    lib.syncLoop(tx.length, function(loop){
      var i = loop.iteration();
      //console.log("block: %s, tx: %s", blockheight, arr_tx[i])
      save_tx(tx[i], blockheight, blockhash,function(){
          //console.log('success')
          loop.next();
      });
    },
...
function save_tx(tx, blockheight, blockhash,cb) {
  txid = tx.txid
            ...
            lib.get_blockhash(block_height, function(blockhash){
              if (blockhash) {
                lib.get_block(blockhash, function(block) {
                  if (block) {
                    if(block.confirmations < settings.confirmations){
                      return cb("Block %s doesn't have enough confirmations yet.", block_height);
                    }
                    arr_tx = [];
