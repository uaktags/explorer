...
function save_tx_array(arr_tx, blockheight, blockhash, cb)
{
  console.dir(arr_tx)
  lib.get_rawtransactionfast(arr_tx, blockhash, function(tx){
    if (tx != 'There was an error. Check your console.') {
    lib.syncLoop(tx.length, function(subTXloop){
      var txit = subTXloop.iteration();
      lib.prepare_vin(tx[txit], function(vin) {
        lib.prepare_vout(tx[txit].vout, tx[txit].txid, vin, function(vout, nvin) {            
          lib.syncLoop(vin.length, function (loop) {
            var i = loop.iteration();
            update_address(nvin[i].addresses, blockheight, tx[txit].txid, nvin[i].amount, 'vin', function(){
              loop.next();
            });
          }, function(){
            lib.syncLoop(vout.length, function (subloop) {
              ...
              if (vout[t].addresses) {
                update_address(vout[t].addresses, blockheight, tx[txit].txid, vout[t].amount, 'vout', function(){
                  subloop.next();
                });
              } else {
                subloop.next();
                    ...
                    async.eachLimit(block.tx, task_limit_txs, function(txid, next_tx) {
                      ...
                      save_tx_array(arr_tx, block_height, blockhash, function(err){
                        if (err) {
                          ...
                          console.log('%s: %s', block_height, arr_tx);
                        }
                      });
                      setTimeout( function(){
                        blockhash = null;
