...
async function update_address_async(hash, blockheight, txid, amount, type) {
  var to_sent = false;
  ...
  await Address.findOneAndUpdate({a_id: hash}, {
    $inc: addr_inc
  }, {
    new: true,
    upsert: true
...
async function save_tx_array(arr_tx, blockheight, blockhash, cb)
{
  lib.get_rawtransactionfast(arr_tx, blockhash, async function(tx){
    if (tx != 'There was an error. Check your console.') {
    console.log("number of txes: ", arr_tx.length)
    lib.syncLoop(tx.length, function(subTXloop){
      var txit = subTXloop.iteration();
      if(blockheight == 73) console.log("tx: ", tx[txit].txid)

      lib.prepare_vin(tx[txit], async function(vin) {
        
        if(blockheight == 73) console.log(vin)
        voutarr = await lib.prepare_vout_async(tx[txit].vout, tx[txit].txid, vin);
        console.log(voutarr)
        vout = voutarr[0];
        nvin = voutarr[1];  
          if(blockheight == 73) console.log('here1')  
          for(i=0; i<vin.length; i++){      
            if(blockheight == 73) console.log('here2')
            await update_address_async(nvin[i].addresses, blockheight, tx[txit].txid, nvin[i].amount, 'vin');
          }
            if(blockheight == 73) console.log('here3')
            //if(blockheight == 73) console.dir(vout)
            for(t=0; t<vout.length; t++){
              if (vout[t].addresses) {
                await update_address_async(vout[t].addresses, blockheight, tx[txit].txid, vout[t].amount, 'vout');
              } 
              //console.log("here %s", tx[txit]);
              if(blockheight == 73)console.log('here4')
              lib.calculate_total(vout, function(total){
                var newTx = new Tx({
                  ...
                  timestamp: tx[txit].time,
                  blockhash: tx[txit].blockhash,
                  blockindex: blockheight,
                });
                newTx.save(function(err) {
                  if (err) {
                    ...
                    if(blockheight == 73) console.log('txid: ', tx[txit].txid);
                    subTXloop.next();
                    //return cb();
                  }
                });
...
function save_tx(txid, blockheight, blockhash,cb) {
  //var s_timer = new Date().getTime();
  lib.get_rawtransactionfast(txid, blockhash, function(tx){
    if (tx != 'There was an error. Check your console.') {
                ...
                lib.get_block(blockhash, function(block) {
                  if (block) {
                    ...
                    async.eachLimit(block.tx, task_limit_txs, function(txid, next_tx) {
                      ...
                      save_tx_array(arr_tx, block_height, blockhash, function(err){
                        if (err) {
                          console.log(err);
                        } else {
                          console.log('block %s: tx: %s', block_height, arr_tx);
                        }
                      });
                      setTimeout( function(){
                        blockhash = null;
