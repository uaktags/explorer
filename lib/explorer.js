...
module.exports = {
  ...
  convert_to_satoshi_async: async function(amount, cb) {
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if ((coin.hasOwnProperty('isValidVOUT') && coin.isValidVOUT(vout[i])) || (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata')) { 
        // check if vout address is unique, if so add it array, if not add its amount to existing index
        //console.log('vout:' + i + ':' + txid);
        var vout_address = '';
        if(coin.hasOwnProperty('useCustomAddresses') && coin.useCustomAddresses){
          vout_address = coin.customAddresses(vout[i].scriptPubKey);
        }else{
          vout_address = vout[i].scriptPubKey.addresses[0];
        }
        module.exports.is_unique(arr_vout, vout_address, function(unique, index) {
          if (unique == true) {
            // unique vout
            module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
              arr_vout.push({addresses: vout_address, amount: amount_sat});
              loop.next();
            });
          } else {
            // already exists
      ...
      if (vout[0].scriptPubKey.type == 'nonstandard') {
        if ( arr_vin.length > 0 && arr_vout.length > 0 ) {
          if ((coin.hasOwnProperty('useCustomAddresses') && coin.useCustomAddresses(arr_vin[0]) == coin.useCustomAddresses(arr_vout[0])) || arr_vin[0].addresses == arr_vout[0].addresses) {
            //PoS
        ...
        if (tx) {
          module.exports.syncLoop(tx.vout.length, function (loop) {
            ...
            if (tx.vout[i].n == input.vout) {
              ...
              var vout_addresses;
              if (coin.hasOwnProperty('useCustomAddresses') && coin.useCustomAddresses){
                vout_addresses = coin.customAddresses(tx.vout[i].scriptPubKey);
              }else{
                if(tx.vout[i].scriptPubKey.addresses){
                  vout_addresses = tx.vout[i].scriptPubKey.addresses[0];
                }else{
                  vout_addresses = false;
                }
              } 
              if(vout_addresses != false) {
                addresses.push({hash: vout_addresses, amount:tx.vout[i].value});  
              }
                loop.break(true);
                loop.next();
              //});
