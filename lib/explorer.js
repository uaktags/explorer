...
module.exports = {
  ...
  is_unique_async: async function(array, object, cb) {
    var unique = true;
    var index = null;
  ...
  calculate_total_async: function(vout) {
    var total = 0;
  ...
  prepare_vout_async: async function(vout, txid, vin){
    var arr_vout = [];
    var arr_vin = [];
    arr_vin = vin;
    if(vout.length != 0){
      for(i=0; i<vout.length; i++){
        ...
        if(coin.isValidVOUT(vout[i])){  
          ...
          uniquearr = await module.exports.is_unique_async(arr_vout, vout[i].scriptPubKey.addresses[0]);
          unique = uniquearr[0];
          index = uniquearr[1];
            if (unique == true) {
              // unique vout
              amount_sat = await module.exports.convert_to_satoshi_async(parseFloat(vout[i].value))
  ...
  get_input_addresses_async: async function(input, vout, cb) {
    var addresses = [];
    if (input.coinbase) {
      var amount = 0;
      for(i=0; i<vout.length; i++){
  ...
  prepare_vin_async: async function(tx, cb) {
    var arr_vin = [];
    for(i=0; i<tx.vin.length; i++){
      addresses = await module.exports.get_input_addresses_async(tx.vin[i], tx.vout); 
        if (addresses && addresses.length) {
          //console.log('vin');
          uniquearr = await module.exports.is_unique_async(arr_vin, addresses[0].hash);
          unique = uniquearr[0];
          index = uniquearr[1];
            if (unique == true) {
